...
Export and import of Forge's own data as a single `.forge` file, plus the entry point for
...
- **No absolute path may enter a `.forge`.** A clip's `imagePath` names a file on the Mac that wrote
...
| `Model/BackupArchive.swift` | Directory ⇄ `.forge`; the only file importing `AppleArchive` |
...
| `Service/RaycastImportReader.swift` | Raycast → Forge field mapping |
...
The old flat `Forge-Settings-*.json` export is gone rather than deprecated, and nothing reads it.
