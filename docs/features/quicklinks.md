...
Quicklinks reuse Forge's one template engine — the same
...
Forge's own dialog and leaves no partial state.
...
independently selectable category. Forge reads `name`, `link`, `createdAt` and the optional
