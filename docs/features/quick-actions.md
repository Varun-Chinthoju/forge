...
- **One funnel, whichever way an action started.** A shortcut and a launcher row both land on
  ...
  palette — once the palette is gone, the frontmost app is Forge, and the action would read its
...
- **Forge is never an event target.** `QuickActionRunner.selection(in:using:)` refuses our own
...
A custom action previews by default, switchable to Replace per row: Forge cannot know whether an
...
`HotKeyManager.start` can prune a binding whose action was deleted while Forge was off.
...
`QuickActionPanel` is Forge's **fourth borderless surface**, beside the dialog, the notes panel
...
Forge asserts, so it is the part worth checking by hand.
...
- Run one from the launcher (⌘Space → "Fix Grammar") with text selected behind it: the palette
  closes and the selection in the displaced app is what gets acted on, not Forge's own field.
...
- Press a shortcut with Forge's own Settings window frontmost: refused, with a HUD.
