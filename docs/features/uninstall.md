...
- Forge refuses to plan its own uninstall, compared against the **running** identity so the Dev channel
  refuses itself too.
...
- **An installed sibling owns its own artifacts.** If any _other_ installed app's bundle ID is a
  longer match for the same component, that app owns it. Without this, uninstalling `com.varun.forge.app`
  would also trash `com.varun.forge.app.beta` and `…​.dev` — separate products that merely share a
...
`UninstallIdentity.make` returns `nil` — refusing the whole uninstall — when the target is Forge
