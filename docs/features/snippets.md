...
Debug (`com.varun.forge.app.dev`), beta, and stable therefore never share snippet files. The storage
...
frontmatter keeps the same identity. Renaming a file outside Forge appears as deletion of the old
...
an independently selectable category. Forge reads `title`, `text` and the optional `keyword` from
...
suffixes as snippets created in Forge, and duplicate keywords are preserved.
...
there are no aliases, so a key Forge does not know names itself in the error.
...
A token Forge cannot parse — an unknown name, an unknown modifier, a duplicated or unsupported
...
duplicates resolve by file identity. Forge-tagged synthetic events are ignored.
...
After either launcher or keyword delivery is confirmed, Forge may show a brief non-activating,
...
for permissions or activates Forge. Failed, cancelled, rejected, or prompt-cancelled expansions do
...
Longer or multiline fallback text uses a temporary paste. Forge snapshots every item, type and data
payload, then **lends a board holding nothing but the expansion** — one item carrying the plain text
and Forge's own marker type — and restores by rewriting the snapshot whole.
...
previous copy instead. A representation Forge cannot rewrite to mean the expansion is one it must
...
synchronizes to Forge's ownership changes so temporary or restored text is not added as new history.
...
any pasteboard restoration still owned by Forge.
...
non-ignored input to `cancelAutomaticExpansion`, and Forge's own tagged synthetic events classify
...
between revalidation and mutation, so Forge does not claim that impossible guarantee.
