...
Stats, Forge itself, Mission Control, Siri, Time Machine, Screenshot, System Information, Font
...
| --- | --- | --- |
| `.userAlias` | the alias the user typed in Forge, for any entry kind | literal |
...
and its `url` carries the destination instead of the catalog's `forge://` placeholder. Nothing
...
`SystemActionCatalog` is a Foundation-only inventory of the macOS actions Forge exposes. Its
...
Toggle System Appearance changes macOS; Forge follows it only while its own Appearance is System.

Restart, Shut Down, Log Out, Empty Trash and Quit All Applications confirm before execution: ↵ runs
the action, Escape cancels. Every dialog is Forge's own: confirmations, failure reports and the Set
...
opened it. Volume and mute actions also show Forge's transient volume HUD, since macOS only draws
...
Forge. See [quick-actions.md](quick-actions.md).
...
> **Invariant:** `Tests/fuzz-test.swift` compiles the real `Forge/Features/Launcher/Model/SearchRelevance.swift`, so
...
- **Quit All Applications** a system action. `AppLauncher.quitAllTargets()` is the
  policy (every `.regular` app except Finder — `terminate()` only relaunches it — and Forge,
