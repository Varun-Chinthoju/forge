...
Rule 1 also delivers the "works for windows Forge never moved" requirement: the capture happens in
...
- **The Dock ignores a gesture from a short-lived process.** The calls all report success and nothing
  happens. Forge is a resident menu-bar app, so this is free — but it is why a one-shot CLI cannot
...
5. Restore on a window Forge has never moved.
