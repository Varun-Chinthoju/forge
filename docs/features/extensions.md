...
Forge runs Raycast extensions: the same `package.json` + prebuilt CommonJS bundles Raycast itself
...
`react/jsx-runtime`, `@raycast/api` and the Node built-ins external. Forge supplies exactly those,
...
`Forge/Resources/RaycastRuntime.generated.js` (~200 KB minified) is **generated and committed**, the
same arrangement as `EmojiData.generated.swift`: building Forge never needs Node. Sources live in
[`Scripts/raycast-runtime/`](../../Scripts/raycast-runtime):

| File | What it is |
| --- | --- |
| `src/index.js` | the `__forge` object Swift calls into (`boot`, `start`, `dispatch`, `popNavigation`, `settle`, `fireTimer`, `stop`) |
...
- **Async** (`invoke`) for anything that needs the main actor — clipboard, toasts, window control,
  `fetch`, `exec`, `oauth`. Swift answers later through `__forge.settle`, so the JS thread never blocks on the
  UI.
...
`Forge/Features/Extensions/`, split the same way as every other feature:
...
- **Search-bar dropdown** — `List.Dropdown` and `Grid.Dropdown` draw as
  ...
  command's own dropdown answers before Forge's clipboard filter can. The list is
...
- **Form** — label-left/control-right rows. Field values live in the extension (React owns them); every
  edit dispatches `onForgeChange` and the resulting re-render is what updates the control, so
...
2. **Import from Raycast** — copies the already-built bundles out of a local Raycast. Nothing is
   ...
   so when Raycast has something Forge doesn't — installing in Raycast otherwise leaves no trace
...
tokens in the login Keychain (service `com.forge.extensions.oauth`, `kSecAttrAccessibleWhenUnlocked`),
scoped per extension and dropped on uninstall.

The redirect address belongs to the extension author's OAuth app registration, so Forge cannot choose
it — it can only be there to catch it. **Forge therefore claims `raycast`, `com.raycast` and `forge`
...
| --- | --- | --- |
| `App` | `raycast://oauth?package_name=Extension` | straight to Forge, no server |
| `AppURI` | `com.raycast:/oauth?package_name=Extension` | straight to Forge, no server |
| `Web` | `https://raycast.com/redirect?packageName=Extension` | through Raycast's page, which reopens a claimed scheme |

Claiming `raycast` means an installed Raycast competes with Forge for those links and macOS picks the
...
this). `ExtensionHostBridge` keeps those inside Forge: `raycast://extensions/<author>/<extension>/<command>`
...
host rather than returning a wrong path. Node's `windows` override is absent: Forge only runs on
...
node build.mjs            # → Forge/Resources/RaycastRuntime.generated.js (commit it)
...
"${TMPDIR:-/tmp}"/forge-harness/ext-test ~/Library/Application\ Support/com.varun.forge.app.dev/extensions/<name> [command]
```
...
| OAuth tokens | macOS Keychain (`com.forge.extensions.oauth`) | yes |
...
**Builds happen in `$TMPDIR/forge-install-<UUID>/`**, named by `ExtensionCleanup.workspace` so the
