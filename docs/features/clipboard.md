...
- **Clipboard writes stamp a private `internalType` marker** so the poller skips Forge's own writes.
...
- **A `.file` entry references the file where it lies and never copies it.** Its absolute path is
  ...
  Forge did not write. `kind` is a plain `TEXT` column, so the case cost no migration; an older
...
- **No recognition ever runs in the app process.** `ClipboardTextWorker` spawns one bundled
  ...
  allocations do not accumulate in Forge. The helper is handed a path and answers with text.
...
tree: a harness that touched `NSPasteboard.general` would land in the reader's own running Forge
...
re-capturing Forge's own writes, every write stamps a private `internalType` marker on the
...
`isCapturing` flag that `prepareForForgePasteboardMutation` reads — so a paste Forge performs
...
A file copied in Finder is recorded as a reference, never as a copy: Forge writes nothing to
...
copy-only for the reverse reason: the path is the user's own file, and Forge must not move it.
