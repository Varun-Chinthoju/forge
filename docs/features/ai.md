...
Forge has one app-wide provider layer for features that need text generation. AI Chat chooses its
...
- **Every request carries Forge's own preamble, and the user's text goes after it.**
  ...
  Forge over anything else. It is not shown in the pane, and `AIPreamble.swift` holds the only
...
- **Installed commands reuse their own login.** Forge launches the user's `codex`, `claude` or
  `opencode` executable without asking for or storing another key. Codex inherits the user's normal
  home and credential-store setting; Claude and OpenCode inherit their normal configuration. Forge
...
- **Claude and OpenCode are text transports, not agents.** Claude runs one turn with no tools, MCP
  ...
  deny-all permissions, disabled sharing and a private working directory; Forge deletes the session
...
completion path. Gemini requests identify Forge through `x-goog-api-client`; OpenRouter requests
carry the app title.

Each connection has an ordered, deduplicated list of exact model identifiers. While its editor is open,
Forge asks the configured provider for the models available to the entered key and uses the result
...
manually. Forge does not ship or guess an API catalog that can become stale. Codex gets its models
...
the only destination Forge can offer the switch to honestly. Picking `None` sends
...
It uses the system SQLite already linked by Forge, stores no provider credentials, and repairs a
...
- Install and sign in to each supported command outside Forge, choose one of its discovered models,
...
commands are never installed by Forge; Settings links to their own install docs and offers a sign-in
...
server uses the same login and credential store as the user's normal Codex command. Forge supplies
...
bubble's status line. System messages become developer instructions alongside Forge's fixed
...
Forge's instructions and bounded conversation history as stdin, consumes newline-delimited JSON,
...
| OpenRouter | `plugins: [{id: "web"}]` — OpenRouter's own layer, any model | `image_url` part, only for models whose catalog lists the `image` modality | never yet — its catalog publishes a `file` modality Forge does not read | `tools` + `role: "tool"` turns |
...
`AIModelCapabilities.documents` is true only for the two HTTP shapes whose bodies Forge writes;
...
Settings → AI is a normal grouped `Form` inside Forge's existing Settings window. Its top AI
...
tools, run commands or touch files. That is a sandbox boundary on a local CLI, not Forge
