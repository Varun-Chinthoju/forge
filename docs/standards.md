...
How code in Forge is written. This is **guidance** — it describes what the codebase already looks
...
one-line call into a layer nobody dares delete. Forge has no external API, no plugin surface and one
...
- **One folder per feature** under `Forge/Features/<Name>/`, holding everything that feature owns —
