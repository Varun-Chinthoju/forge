...
The design system for Forge's UI, written so an agent restyling or extending it stays consistent
with what's already there. This documents **Forge as built** — every rule here maps to code in
`Forge/`. `DesignSystem/Theme.swift` is the single design-token source.
...
Forge is a **command palette**: a borderless floating panel whose surface is just the
...
- **No `NSAlert`, no `NSSlider`, no system popovers.** Every confirmation, failure report, value prompt and transient readout is Forge's own SwiftUI surface (see "Dialogs & HUD"). An Aqua alert on an alpha-over-vibrancy app reads as a different product, and its `runModal` run loop keeps Carbon hotkeys firing underneath.
...
Source: `Forge/DesignSystem/Theme.swift`.
...
Forge owns its dialogs; `NSAlert` is never used. `DialogController` is owned by `AppCore` (the
...
writes `AppleShowScrollBars = WhenScrolling` into Forge's own defaults domain, which outranks the
...
countdown and buttons around it are Forge's own. Its buttons are a **deliberate copy** of
...
Its placeholder is drawn by Forge, not by the field's `prompt` — an `NSTextField` renders a prompt
...
[features/palette.md](features/palette.md#the-placeholder-is-forges-not-the-fields).
