...
Select some text in any app, press a shortcut, and Forge works on it. The result either replaces
...
explains what it does, then asks for [Accessibility](/docs/permissions), because Forge has to read
...
first and the action works on the app behind it, not on Forge's own search field.
...
prompt; Forge cannot tell whether a prompt edits your text or answers a question about it.

The pencil button edits it. Deleting one asks first, then frees its shortcut.

## Changing a built-in prompt

The pencil beside **Fix Grammar**, **Rewrite** or **Summarize** opens the exact prompt Forge uses.
...
- **In Forge's own windows.** Pressing a shortcut with Settings in front does nothing, and says so.
...
Forge first asks the app for the selected text through Accessibility. Chrome, Electron apps and VS
Code get a nudge first, because they only share that once something asks.

If that gives nothing, Forge briefly copies the selection with <kbd>⌘</kbd><kbd>C</kbd>, reads it,
