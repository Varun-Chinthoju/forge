...
Swedish and Icelandic, so Forge will not guess.
...
Restarting Forge never fetches again early, so opening it with a fresh copy makes **zero**
network requests.
...
It all works offline. Forge does not look cities up online.
