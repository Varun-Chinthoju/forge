...
<kbd>⌘</kbd><kbd>Q</kbd> does nothing here, so **no key press over Notes can quit Forge** by
accident.

## Files

Notes live in Forge's Application Support folder:

```
~/Library/Application Support/com.varun.forge.app/Notes/
```
...
**A save overwrites whatever is on disk.** Forge does not watch the folder. If you edit the note
that is _open in Forge_ in another app, that edit is lost at the next autosave.
