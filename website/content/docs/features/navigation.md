...
If the app quit between opening the list and pressing <kbd>return</kbd>, Forge tells you instead of
doing nothing.
...
- **Only items you could click are listed.** Disabled items, separators and hidden items are left
  out.
- **Forge never opens menus to read them.** Some apps build a submenu only when you open it, so
...
| Disabled Applications | Empty   | Apps whose menus Forge will not read at all      |
