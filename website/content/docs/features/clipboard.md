...
Forge keeps what you copy, so you can find it again and paste it back into the app you were
using.
...
- **Clipboard History** in the Forge menu bar menu.
...
- **Files you copy in Finder.** Forge saves a reference to the file where it is, never a copy. Up
...
**Search text in images and PDFs** is **off** by default. Turn it on and Forge reads the text in
...
- It runs in a separate helper process, so Forge itself stays light.
...
**Disabled Applications** starts with Keychain Access and Passwords, so Forge never records what
you copy out of a password manager. Add your own. Copies that apps mark as secret are skipped too.

## Limits worth knowing

History lives in `clipboard.sqlite3`, with images beside it, in Forge's Application Support folder.
It is there, not in Caches, so Time Machine backs it up and macOS never clears it behind your back.

- Forge checks the clipboard twice a second. Its own pastes are marked and skipped, so pasting
  from Forge never adds a new entry.
