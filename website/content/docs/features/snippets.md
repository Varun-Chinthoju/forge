...
One Markdown file per snippet, in Forge's Application Support folder. Frontmatter is optional.
...
**A token Forge cannot read is left in the text exactly as you wrote it**, never silently dropped.
...
- Just before it deletes the keyword and inserts the text, Forge checks everything again. If
  anything changed, what you typed is left alone.

Settings shows the status plainly: **Off**, **Needs Accessibility**, or **Active**.

A keyword typed into Forge's own search field or Settings never expands. A keyword typed in
[Notes](/docs/features/notes) expands right into the note.

### How the text gets there

First choice: one clean replacement through Accessibility, which Forge then checks actually
happened.

Some apps, like Chrome, VS Code, Slack and other Electron apps, do not really accept that, so
Forge types the text instead. Short, single-line expansions of up to 100 characters are typed as
keystrokes.

Longer or multi-line text uses a quick temporary paste. Forge saves your clipboard, pastes only
