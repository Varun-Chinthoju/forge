...
Forge requests **no file permission** for this. Hidden files and the insides of app bundles are
...
in the last 3 days, inside your search scopes. This comes from macOS's own records. Forge keeps no
history of its own.
...
**Forge never adds `~/Library` on its own.** If you add a folder inside it yourself, you get what
you asked for.
