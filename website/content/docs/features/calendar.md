...
Forge reads the calendars already on your Mac, finds the join link in each meeting, and puts it
one keystroke away. It works with any calendar macOS knows about, like iCloud, Google or Exchange.

**Settings → Calendar → Join meetings from Forge** holds the switch. It ships **off**. Turning it
...
Forge looks in the event's URL, location and notes, in that order. It knows **Zoom**,
...
The calendar gets its **own** menu bar item, separate from the Forge icon. You can show either,
both or neither.
