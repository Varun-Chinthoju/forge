...
A backup is one `.forge` file. You tick what goes into it, and you tick again what comes back out
...
**The format belongs to Forge and may change between versions. The only promise is that a backup
imports into the same version that made it.**

The file records which version wrote it. A Forge that does not recognize it says so clearly,
...
Everything a backup carries is also an ordinary file, in `~/Library/Application Support/com.varun.forge.app/`:
...
read them without Forge. That is the point of keeping them that way.
