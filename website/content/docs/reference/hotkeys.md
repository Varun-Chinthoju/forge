...
**Forge ships with nothing bound.** Every global shortcut is one you record.

## What can have a shortcut

- The palette itself (**App Launcher**, in Settings → General)
- Every built-in command, except Open in Browser, Run Shell Command and Quit Forge. See
...
Forge only watches for double taps **while at least one shortcut uses one**, so if you never do,
it costs nothing.
...
Function keys are not offered. Their media functions fire before Forge can see them, so turning F1
...
pick another key, when Forge quits, and it never survives a restart.

In the brief moment before the remap takes hold, the Caps Lock light can still switch. That is the
hardware, not something Forge can stop.

Hyper needs Accessibility and never asks for it on its own. Forge keeps checking, starts working as
...
Shortcuts for things you deleted while Forge was not running are cleaned up the next time it
starts.
