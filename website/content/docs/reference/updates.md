---
title: Updates
description: How Forge finds, checks and installs new versions by itself.
---

Forge keeps itself up to date. There is nothing to set up.

## How it works

1. **Once a day**, Forge asks GitHub whether a newer release is out for its channel. If a check
...
4. Forge checks the download, installs it, and offers to **relaunch**.
...
launcher, the Forge menu bar menu, and **Settings → About**.
...
On an Intel Mac, Forge only installs the universal build. If a release has none, nothing is
...
- **The signature must prove the download is Forge**, signed the same way as the copy you are
  running.
- The app identifier and the version must be the ones expected.
- Forge never runs `xattr` or asks for an administrator password. If `/Applications` cannot be
...
Forge's Homebrew casks tell Homebrew that the app updates itself. So `brew upgrade` **skips
Forge on purpose**: it never reports it as outdated, never downloads it again, and never rolls a
...
Forge is free and open source. **Support Forge** in the launcher, the menu bar and
