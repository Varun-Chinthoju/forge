...
**Volume.** Up and Down move along a **5% grid**: from 37%, up goes to 40% and down to 35%. Forge
...
palette closed, work on the app that is actually in front. Quit All leaves Finder and Forge
running, and quits politely, so apps with unsaved work still ask you to save.

**Toggle System Appearance** changes **macOS itself**, not just Forge. Forge follows along only
