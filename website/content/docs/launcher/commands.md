...
Commands are things Forge does, reachable by name from the launcher. Some are built in. Others
...
| Commands                                           | Calculator History · [Open Camera](/docs/features/camera) · Export Backup · Import Backup · Import from Raycast · Check for Updates · Settings · About Forge · Support Forge · Quit Forge |
...
which need text to work on, and Quit Forge, so no stray key press can quit the app.
...
| Environment | Inherited, plus `FORGE=1`                        |

**There is no Terminal window and no timeout.** Forge never stops a running command on its own,
and a command keeps running if Forge quits. The one exception is the **Stop** button in the output
window.

If the Run In folder no longer exists, the command does not run, and Forge tells you which folder
...
`FORGE=1` lets your `.zshrc` skip that work:
...
`FORGE=1` check above keeps it quiet.
