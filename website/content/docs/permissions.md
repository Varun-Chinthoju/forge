---
title: Permissions
description: What Forge asks for, why, and the moment it asks.
---

Forge asks for a permission **only when you use a feature that needs it**, never at launch. The
...
macOS calls this "control your computer". In Forge it lets the app read from, and write to, the
...
- Forge uses a **listen-only** event tap under the Accessibility grant. It does not use Input
  Monitoring.
...
Turning it on shows Forge's own explanation first, then the macOS prompt. The feature only counts
...
If you say no, Forge tells you and links to the right System Settings pane instead of silently
doing nothing.

## Full Disk Access

Forge **checks** for Full Disk Access but **never asks** for it.
