---
title: Extensions
description: Forge runs Raycast extensions natively, drawn with SwiftUI.
---

Forge runs Raycast extensions: the same `package.json` and the same built command files, drawn
natively in the palette.

There is no Electron, no browser and no Node.js running inside Forge. Extensions run in
...
leave it.** That is the only ongoing cost Forge has.
