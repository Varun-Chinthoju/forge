...
Forge looks in both `~/.config/raycast` and `~/.config/raycast-x`, and an extension found in both
is offered once. There is an **Import All** button. The pane checks again whenever you open it, and
tells you when Raycast has something Forge does not.
...
An app opened from the Dock does not see your terminal's `PATH`, so Forge looks in the usual places
...
Everything lives in Forge's Application Support folder, and **uninstalling an extension removes
