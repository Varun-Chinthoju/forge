...
extension UTType {
    ...
    static let forgeBackup = UTType(exportedAs: "com.forge.backup")
}
...
enum BackupActions {
    ...
    // MARK: - Forge native (own file panels; dialogs come from `AppCore`)
    ...
    static func chooseBackupFile() -> URL? { chooseFile(ofType: .forgeBackup) }
    ...
    {
        guard let destination = chooseSaveLocation(named: "Forge", type: .forgeBackup) else {
