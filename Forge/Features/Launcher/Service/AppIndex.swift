...
extension AppEntry {
    ...
    init(_ layout: WindowLayout) {
        self.init(
            ...
            url: URL(string: "forge://window-layout/" + layout.id.uuidString)!,
    ...
    init(_ action: CustomQuickAction) {
        self.init(
            ...
            url: URL(string: "forge://quick-action/" + action.id.uuidString)!,
    ...
    init(_ quicklink: Quicklink) {
        self.init(
            ...
            url: URL(string: "forge://quicklink/" + quicklink.id.uuidString)!,
...
final class AppIndex {
    ...
    private static let systemActionEntries: [AppEntry] = SystemActionCatalog.all
        .map { command in
            AppEntry(
                ...
                url: URL(string: "forge://system-action/" + command.id.rawValue)!,
    ...
    private static let allWindowCommandEntries: [AppEntry] = WindowCommandCatalog.all
        .map { command in
            AppEntry(
                id: command.entryID, name: command.name,
                url: URL(string: "forge://window-command/" + command.id.rawValue)!,
    ...
    func setCustomCommands(_ commands: [CustomCommand]) {
        let entries = commands.filter(\.isEnabled).map { command in
            AppEntry(
                id: command.entryID, name: command.name,
                url: URL(string: "forge://custom-command/" + command.id.uuidString)!,
