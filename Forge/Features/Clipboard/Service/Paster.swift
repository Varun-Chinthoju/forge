...
enum Paster {
    /// Stamped on Forge's own synthetic keystrokes so the snippet keyword tap can skip them.
    ...
    private static func postCommand(key: CGKeyCode, toPid pid: pid_t?) {
        ...
        down.setIntegerValueField(.eventSourceUserData, value: forgeEventTag)
        up.setIntegerValueField(.eventSourceUserData, value: forgeEventTag)
