...
enum ExtensionCommandMode: String, Sendable, Codable {
    ...
    var unsupportedReason: String? {
        self == .menuBar
            ? "Menu bar commands aren't supported yet — Forge only runs view and no-view commands."
            : nil
    }
}
...
enum ExtensionPreferenceValue: Sendable, Hashable {
    ...
    init?(manifestDefault raw: Any?) {
        ...
        case let value as [String: Any]:
            // Platform-keyed default; Forge is macOS-only.
...
/// A Raycast extension's `package.json`, reduced to what Forge uses.
