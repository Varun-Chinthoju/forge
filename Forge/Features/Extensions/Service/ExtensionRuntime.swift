...
final class ExtensionRuntime: @unchecked Sendable {
    private let queue = DispatchQueue(label: "com.forge.extensions.js", qos: .userInitiated)
    ...
    func stop(session: String) async {
        await onQueue { context in
            _ = context.objectForKeyedSubscript("__forge")?
    ...
    /// Installs the legacy `__tinycastHost` JS→Swift seam and compiler hook.
