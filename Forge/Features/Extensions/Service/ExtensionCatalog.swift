...
enum ExtensionCatalog {
    ...
    private static func supportDirectory() -> URL {
        ...
        let bundleID = Bundle.main.bundleIdentifier ?? "com.varun.forge.app"
    ...
    enum InstallError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .noBuiltCommands(let name):
                return
                    "\(name) has no built command bundles. Forge installs prebuilt extensions — run `ray build` in the extension folder first, or import one from an installed Raycast."
