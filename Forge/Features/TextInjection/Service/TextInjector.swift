...
final class TextInjector {
    ...
    /// A hotkey's target comes from `frontmostApplication`, which can be Forge itself.
    ...
    func copySelection(
        ...
        clipboardManager.prepareForForgePasteboardMutation()
    ...
    private func restore(_ snapshot: PasteboardSnapshot, to pasteboard: any PasteboardAccess) {
        ...
        clipboardManager.synchronizeAfterForgePasteboardMutation(
    ...
    private func finish(_ lease: TemporaryPasteboardLease) {
        ...
        case .restored(let changeCount):
            ...
            clipboardManager.synchronizeAfterForgePasteboardMutation(changeCount: changeCount)
        case .superseded:
            break
        case .failed:
