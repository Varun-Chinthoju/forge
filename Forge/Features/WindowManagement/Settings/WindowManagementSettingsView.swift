    ...
    var body: some View {
        ...
        return Form {
            FeatureSwitchSection(
                ...
                enableSubtitle:
                    "Moves the window you were last in, using the Accessibility permission Forge already uses to paste.",
...
private struct WindowCommandSettingsRow: View {
    ...
    private var entry: AppEntry {
        AppEntry(
            ...
            url: URL(string: "forge://window-command/" + command.id.rawValue)!, bundleID: nil,
            kind: .windowCommand)
    }
