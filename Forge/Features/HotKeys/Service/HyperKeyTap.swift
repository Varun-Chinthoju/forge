...
private enum CapsLockRemap {
    ...
    private static let queue = DispatchQueue(label: "com.forge.capslock-remap", qos: .utility)
    ...
    private static func apply(_ mapping: String) {
        ...
        do {
            ...
            if process.terminationStatus != 0 {
                NSLog("Forge: hidutil remap exited %d", process.terminationStatus)
            }
        } catch {
            NSLog("Forge: hidutil caps lock remap failed: %@", error.localizedDescription)
        }
    }
}
