...
enum ShellCommandRunner {
    ...
    private static let queue = DispatchQueue(
        label: "com.forge.shell-command", qos: .userInitiated, attributes: .concurrent)
    ...
    nonisolated private static func execute(
        ...
        // Lets a shell config skip slow sections when Forge is the caller.
        process.environment = ProcessInfo.processInfo.environment.merging(["FORGE": "1"]) { _, new in
            new
        }
    ...
    nonisolated private static func shellArguments(
        ...
        [loadingShellEnvironment ? "-ilc" : "-lc", command, "forge"] + arguments
    }
    ...
    private final class StreamCapture: @unchecked Sendable {
        ...
        static func make() -> StreamCapture? {
            let template = FileManager.default.temporaryDirectory
                .appendingPathComponent("forge-command-stream.XXXXXX").path
