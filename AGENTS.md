...
**Forge targets one macOS — the current stable release — and nothing else.** macOS 26+, the Xcode 26
...
| Folder | Holds |
| --- | --- |
| `Forge/App/` | `@main`, `AppDelegate`, `AppCore` — the composition root |
| `Forge/DesignSystem/` | shared visual primitives; `Theme.swift` is the only design-token source |
| `Forge/Platform/` | system shims: `Permissions`, `AppPaths`, `Signposts`, `NotificationToken`, … |
| `Forge/Palette/` | the palette shell: panel, window controller, `RootPaletteView`, `PaletteScreen` |
| `Forge/Windows/` | the non-palette AppKit surfaces: `Dialog/`, `HUD/`, `About/`, `AppWindowController` |
| `Forge/Features/` | one folder per feature; larger ones split `Model/` `Service/` `UI/` `Settings/` |
...
- **Forge presents its own dialogs — never `NSAlert`, `NSSlider` or a system popover.** A question
...
- **Debug builds are their own channel** — `Forge Dev.app` / `com.varun.forge.app.dev` — so a local run
...
- **XcodeGen owns the project.** `Forge.xcodeproj` is committed but generated from `project.yml`;
...
- `grep -rln 'import AppKit\|import SwiftUI\|import Cocoa' Forge/Features/*/Model/` returns nothing.
