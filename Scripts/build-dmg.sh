#!/bin/bash
# Build a signed Forge.app into build/Forge-<version>.dmg. Usage: ./Scripts/build-dmg.sh [version]
set -euo pipefail

cd "$(dirname "$0")/.." || exit 1
export DEVELOPER_DIR="${DEVELOPER_DIR:-/Applications/Xcode.app/Contents/Developer}"
IDENTITY="Forge Self-Signed"
...
echo "▸ Building signed Forge.app (Release)…"
...
APP="$DERIVED/Build/Products/Release/Forge.app"
...
rm -f "$DMG"
diskutil image create from "$STAGE" --format UDZO --volumeName "Forge" "$DMG" >/dev/null
rm -rf "$STAGE"
