...
REPO="${REPO:-Varun-Chinthoju/forge}"
CHANNEL="${CHANNEL:?CHANNEL is required (beta|stable)}"
TAG="${TAG:?TAG is required, e.g. v0.9.13-beta.61}"
SHA="${SHA:-$(git rev-parse HEAD)}"
VERSION="${VERSION:-${TAG#v}}"
DISPLAY_NAME="${DISPLAY_NAME:-Forge}"
BUNDLE_ID="${BUNDLE_ID:-com.varun.forge.app}"
CASK="${CASK:-forge}"
...

{
    ...
    printf '```sh\nbrew trust --tap Varun-Chinthoju/forge\nbrew install --cask Varun-Chinthoju/forge/%s\n```\n' "$CASK"
    # The stable DMG is arm64-only; macOS 26 is the last release that boots on Intel.
    if [ "$CHANNEL" = "stable" ]; then
        printf '%s\n' "On an **Intel** Mac, install \`Varun-Chinthoju/forge/forge-universal\` instead — same app, built with both slices."
    fi
