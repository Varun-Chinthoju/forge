...
struct ExtensionCleanupTests {
    ...
    static func sweepTakesOnlyOurWorkspaces() {
        ...
        makeDirectory(roots.temp, "forge-install-ABC123")
        makeDirectory(roots.temp, "forge-install-DEF456")
        makeDirectory(roots.temp, "keep-me")
        makeFile(roots.temp, "unrelated.txt")

        ExtensionCleanup.sweepWorkspaces(in: roots.temp)

        expect(!exists(roots.temp, "forge-install-ABC123"), "a stale workspace is swept")
        expect(!exists(roots.temp, "forge-install-DEF456"), "every stale workspace is swept")
    ...
    static func cleanTakesOnlyOrphans() {
        ...
        makeDirectory(roots.temp, "forge-install-STRAY")
        ...
        expect(!exists(roots.temp, "forge-install-STRAY"), "a stray workspace goes")
