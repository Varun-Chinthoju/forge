...
struct MCPStdioTests {
    ...
    static func anUnsolicitedServerRequestIsDeclined() async {
        ...
        expect(
            ...
            "a server asking Forge for something is declined without derailing the handshake")
        connection.stop()
    }
...
final class StubMCPServer {
    ...
    init?(mode: String = "normal") {
        ...
        let executable = root.appending(path: "bin/forge-mcp-stub")
        do {
        ...
        server = MCPServer(
            name: "Stub", slug: "stub",
            transport: .stdio(
                command: "forge-mcp-stub", arguments: [], environmentKeys: []))
    }
