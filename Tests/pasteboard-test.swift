...
// reader's own running Forge as a genuine copy.
import AppKit
...
struct PasteboardTests {
    ...
    static func everyFileFlavourIsRead() {
        withScratch { dir in
            let file = dir.appendingPathComponent("Forge-Settings-2026-08-20.json")
            ...
            text.setString("Forge-Settings-2026-08-20.json", forType: .string)
            expect(
    ...
    static func volatileAndMissingFilesFallThrough() {
        let temp = URL(fileURLWithPath: "/private/tmp/forge-volatile-\(UUID().uuidString).png")
    ...
    static func withScratch(_ body: (URL) -> Void) {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("forge-pasteboard-test-\(UUID().uuidString)", isDirectory: true)
