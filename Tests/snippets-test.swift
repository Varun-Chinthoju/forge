...
struct SnippetsTests {
    ...
    private static func testRaycastImport() {
        ...
        check(
            "Raycast import uses safe Forge defaults",
    ...
    private static func testRepositoryStorage() throws {
        ...
        let root = fm.temporaryDirectory.appendingPathComponent(
            "forge-snippets-tests-\(UUID().uuidString)",
            isDirectory: true)
        ...
        let stable = SnippetRepository(
            bundleIdentifier: "com.varun.forge.app",
            applicationSupportRoot: channelRoot)
        let beta = SnippetRepository(
            bundleIdentifier: "com.varun.forge.app.beta",
            applicationSupportRoot: channelRoot)
        let dev = SnippetRepository(
            bundleIdentifier: "com.varun.forge.app.dev",
            applicationSupportRoot: channelRoot)
    ...
    private static func testRepositoryConcurrency() async throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory.appendingPathComponent(
            "forge-snippets-concurrency-\(UUID().uuidString)",
            isDirectory: true)
        try fm.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fm.removeItem(at: root) }
    ...
    private static func testCopySelectionFallback() async {
        ...
        let backing = NSPasteboard(name: .init("forge-copy-tests-\(UUID().uuidString)"))
    ...
    private static func testDeliveryQueueAndPasteboard() async throws {
        ...
        let backingPasteboard = NSPasteboard(
            name: .init("forge-snippets-tests-\(UUID().uuidString)"))
        ...
        check(
            "pasteboard restoration leaves no Forge marker on the restored clipboard",
    ...
    private static func testStoreWatcher() async throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory.appendingPathComponent(
            "forge-snippets-watcher-\(UUID().uuidString)",
            isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let repository = SnippetRepository(
    ...
    private static func testDynamicPlaceholders() {
        ...
        // Raycast's snippet spelling resolves like Forge's.
    ...
    private static func testKeywordPolicy() {
        ...
        check("synthetic Forge events are classified as ignored", syntheticInput == .ignored)
    ...
    private static func testKeywordListenerLifecycle() {
        ...
        listener.processEvent(
            ...
            secureEventInputEnabled: false)
        check(
            "real user input invalidates pending automatic delivery while Forge events do not",
            activityCount == 1)
...
final class ClipboardManager {
    static let internalType = NSPasteboard.PasteboardType("com.forge.internal")
    func prepareForForgePasteboardMutation() {}
    func synchronizeAfterForgePasteboardMutation(changeCount: Int) {}
}
...
enum Paster {
    static let forgeEventTag: Int64 = 0x54494E59
