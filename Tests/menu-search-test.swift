...
struct MenuSearchTests {
    ...
    static func targetClassify() {
        ...
        expect(
            classify("Forge", isSelf: true, hasMenuBar: false) == .selfTarget,
            "self wins over the menu-bar check, since Forge itself runs accessory")
        expect(
        ...
        expect(
            classify("Forge", isSelf: true, isExcluded: true) == .selfTarget,
            "self still wins: Forge has no menu to exclude in the first place")
    }
